...
const sleepiestMinute = Object.keys(hoursLog).reduce(
  (acc, guardId) => {
    const minsForGuard = hoursLog[guardId]["mins"];
    const highestMinForGuard = Object.keys(minsForGuard).reduce(
      (acc, minute) => {
        if (minsForGuard[minute] > acc.count) {
          acc = { count: minsForGuard[minute], minute };
        }
        return acc;
      },
      { count: 0, minute: null }
    ...
    if (highestMinForGuard.count > acc.count) {
      acc = {
        guardId,
        minute: highestMinForGuard.minute,
        count: highestMinForGuard.count
      };
    }
    return acc;
  },
  { guardId: null, minute: null, count: 0 }
);

console.log("Sleepiest minute:", sleepiestMinute);
console.log(
  "Puzzle result:",
  Number(sleepiestMinute.guardId) * Number(sleepiestMinute.minute)
);

// console.log(`Sleepiest guard at min ${minToExamine}`, sleepiestGuardAtMin);
// console.log(
//   "Puzzle result:",
//   Number(sleepiestGuardAtMin.guardId) * minToExamine
// );
